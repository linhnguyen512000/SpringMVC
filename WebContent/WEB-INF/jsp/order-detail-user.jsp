...
<!-- 			<h1 class="tt-title-subpages noborder">ORDER DETAIL</h1> -->
			<div class="tt-shopping-layout">
				<h2 class="tt-title-border">
					Đơn hàng #<span  id="order_id">${order.id}</span> - <span  id="delivery_status">${order.deliveryStatus}</span>
					<span class="ordered_date">Ngày đặt hàng: <span  class="date">${order.createdAt }</span></span>
				</h2>
					...
					<div class="tt-table-responsive">
						<table class="tt-table-shop-01">
							...
							<c:forEach var="detail" items="${list}"> 
								<tr >
									<td><a href="${pageContext.request.contextPath}/detail/${detail.product.id}">${detail.product.name }</a></td>
									<td >${detail.size }</td>
									<td >${detail.product.price } VND</td>
									<td >${detail.quantity }</td>
									<td  class="text-right price">${detail.quantity*detail.product.price } VND</td>
								</tr>
							</c:forEach>
								<tr>
									<td colspan="4" class="lbl_total_amount text-right">Tổng cộng</td>
									<td  class="total_amount text-right">${order.totalAmount } VND</td>
								</tr>
							</tbody>
						</table>
					</div>
...
<script src="${pageContext.request.contextPath}/js/order-detail.js" type="text/javascript"></script>
</body>
</html>